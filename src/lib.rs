...
impl<E: Debug, CE: OutputPin<Error = E>, SPI: SpiDevice<u8, Error = SPIE>, SPIE: Debug> NRF24L01<E, CE, SPI> {
	...
	pub fn set_frequency(&mut self, freq_offset: u8) -> Result<(), Error<SPIE>> {
	...
	pub fn set_rf(&mut self, rate: &DataRate, power: u8) -> Result<(), Error<SPIE>> {
		assert!(power < 0b100);
	...
	/// Set CRC mode
	pub fn set_crc(&mut self, mode: CrcMode) -> Result<(), Error<SPIE>> {
	...
	pub fn set_pipes_rx_enable(&mut self, bools: &[bool; PIPES_COUNT]) -> Result<(), Error<SPIE>> {
		self.write_register(EnRxaddr::from_bools(bools))?;
		Ok(())
	}

	/// Set address `addr` of pipe number `pipe_no`
	pub fn set_rx_addr(&mut self, pipe_no: usize, addr: &[u8]) -> Result<(), Error<SPIE>> {
		macro_rules! w {
            ( $($no: expr, $name: ident);+ ) => (
                match pipe_no {
                    $(
	...
	pub fn set_tx_addr(&mut self, addr: &[u8]) -> Result<(), Error<SPIE>> {
	...
	pub fn set_auto_retransmit(&mut self, delay: u8, count: u8) -> Result<(), Error<SPIE>> {
	...
	pub fn get_auto_ack(&mut self) -> Result<[bool; PIPES_COUNT], Error<SPIE>> {
		// Read
	...
	pub fn set_auto_ack(&mut self, bools: &[bool; PIPES_COUNT]) -> Result<(), Error<SPIE>> {
		// Convert back
	...
	pub fn get_address_width(&mut self) -> Result<u8, Error<SPIE>> {
		let (_, register) = self.read_register::<SetupAw>()?;
		Ok(2 + register.aw())
	}

	/// Set address width configuration
	pub fn set_address_width(&mut self, width: u8) -> Result<(), Error<SPIE>> {
	...
	pub fn get_interrupts(&mut self) -> Result<(bool, bool, bool), Error<SPIE>> {
